...
plot.os_loadings <- function(output, plot_vars, var_sel, pldim) {
  ...
  if (ndim == 1) {
    os_data <- output$os_data
    nobj <- nrow(os_data)
    nvar <- ncol(os_data)
    os_svd <- svd(os_data, nu = pldim, nv = pldim)
    sval <- os_svd[[1]][1:pldim]
    sval <- diag(sval / nobj**.5)
    os_loadings <- os_svd[[3]] %*% sval
    varnames <- colnames(os_data)
    rownames(os_loadings) <- varnames
  ...
  minx <- min(os_loadings[, 1] * 1.5)
  maxx <- max(os_loadings[, 1] * 1.5)
  miny <- min(os_loadings[, 2] * 1.5)
  maxy <- max(os_loadings[, 2] * 1.5)
  xlab <- "dimension 1"
  ylab <- "dimension 2"
  main <- "os component loadings"
  # dev.new(width=10, height=10, unit="cm")
  plot(rbind(os_loadings * 1.05, os_loadings * -.50),
    type = "n", pch = 20,
    xlab = xlab, ylab = ylab, main = main, cex = 0.5, asp = 1, axes = FALSE
  )
  r <- c(1:4) / 4
  axis(1, c(rev(r * -1), 0, r))
  axis(2, c(rev(r * -1), 0, r), las = 2)
  box()
...
plot.os_catquants <- function(output, plot_vars, rcplot) {
  os_loadings <- output$os_loadings
  Title <- rownames(os_loadings)
  os_catquants <- output$os_catquants
  c_ncat <- output$c_ncat
  level <- output$level
  ...
  for (j in plot_vars) {
    invar <- j
    inj <- (c_ncat[invar] + 1):c_ncat[invar + 1]
    ...
    clm <- ceiling(max(abs(os_catquants[inj, ])))
    r <- c(1:(2 * clm)) / 2
    axis(1, 1:length(inj))
    axis(2, c(rev(r * -1), 0, r), las = 2)
    box()
...
plot.ordering <- function(output, plot_var, which_plot = 1) {
  c_ncat <- output$c_ncat
  os_catquants <- output$os_catquants
  invar <- plot_var
  inj <- (c_ncat[invar] + 1):c_ncat[invar + 1]
  pl1 <- cbind(as.vector(c(1:length(inj))), os_catquants[inj, ])
  Title <- rownames(output$os_loadings)

  if (which_plot == 1) {
    title <- c("transformation", Title[invar])
    plot(type = "n", pl1, xlab = "original categories", ylab = "optimal quantifications", main = title, axes = FALSE)
    clm <- ceiling(max(abs(os_catquants[inj, ])))
    r <- c(1:(2 * clm)) / 2
    axis(1, 1:length(inj))
