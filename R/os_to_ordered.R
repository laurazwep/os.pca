...
os_to_ordered <- function(os_object, data, trans_list = NULL) {
  ...
  os_cols <- rownames(os_object$os_loadings)[os_object$level != "numerical"]
  ...
  # translate input data with os_levels
  cat_data <- data
  ...
  for (column in os_cols) {
    ...
    if (anyNA(match(data[, column], levels_os))) {
      unseen <- unique(data[is.na(match(data[, column], levels_os)), column])
      warning(sprintf("Column '%s': %d value(s) not found in levels and set to NA: %s",
        column, length(unseen), paste(unseen, collapse = ", ")))
    }

    cat_data[, column] <- factor(data[, column], levels = levels_os, ordered = TRUE)
  }
